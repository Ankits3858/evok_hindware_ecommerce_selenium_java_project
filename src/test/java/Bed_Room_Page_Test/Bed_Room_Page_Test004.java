...
public class Bed_Room_Page_Test004 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Bed_Room_color_Dropdown_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(bp.get_Bed_Room_color_dropdown());
        logger.info("successfully clicked on the Bed_Room color dropdown");
        logger.info("VERIFYING the checkboxes inside 'Bed_Room color dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("all checkboxes inside 'Bed_Room color type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Bed_Room color type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("ALL checkboxes inside 'Bedroom color dropdown' are enabled");
