...
public class Wardrobe_Page_Test004 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Wardrobe_Page_color_filter_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       // cu.scroll_down();
        ...
        cu.click_on_element(wp.get_Wardrobe_color_dropdown());
        logger.info("successfully clicked on the Wardrobe color dropdown");
        logger.info("VERIFYING the checkboxes inside 'Wardrobe color dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("ALL checkboxes inside 'Wardrobe color type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Wardrobe color type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("ALL checkboxes inside 'Wardrobe color dropdown' are enabled");
    }
    @BeforeMethod
