...
public class Wardrobe_Page_Test007 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Wardrobe_Page_Sort_by_filter_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
      ...
      //  cu.scroll_down();
        ...
        cu.click_on_element(wp.get_Wardrobe_sort_by_dropdown());
        logger.info("successfully clicked on the Wardrobe Sort_by dropdown");
        logger.info("VERIFYING the checkboxes inside 'Wardrobe Sort_by dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("(//select[@id='sorter'])[1]//child::option");
        logger.info("ALL checkboxes inside 'Wardrobe Sort_by type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Wardrobe Sort_by type dropdown' enabled or not");
        cu.are_checkboxes_enabled("(//select[@id='sorter'])[1]//child::option");
        logger.info("ALL checkboxes inside 'Wardrobe Sort_by dropdown' are enabled");
