...
public class Lightning_Decor_Page007 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Lightning_Decor_Page_Test_sort_by()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(ld.get_Lightning_Decor_sort_by_dropdown());
        logger.info("successfully clicked on the Lightning_Decor_Page sort_by dropdown");
        logger.info("VERIFYING the checkboxes inside 'Lightning_Decor_Page sort_by dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("(//select[@id='sorter'])[1]//child::option");
        logger.info("ALL checkboxes inside 'Lightning_Decor_Page sort_by type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Lightning_Decor_Page sort_by type dropdown' enabled or not");
        cu.are_checkboxes_enabled("(//select[@id='sorter'])[1]//child::option");
        logger.info("ALL checkboxes inside 'Lightning_Decor_Page sort_by dropdown' are enabled");
