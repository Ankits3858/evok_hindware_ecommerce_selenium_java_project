...
public class Dining_Page_Test006 extends Baseclass {
    public WebDriver driver;

    @Test(retryAnalyzer = My_Retry.class)
    public void Dining_Page_Primary_material_filter_test() {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       // cu.scroll_down();
        ...
        cu.click_on_element(D.get_Dining_primary_material_dropdown());
        logger.info("successfully clicked on the Dining Primary_material dropdown");
        logger.info("VERIFYING the checkboxes inside 'Dining Primary_material dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("ALL checkboxes inside 'Dining Primary_material type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Dining Primary_material type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("ALL checkboxes inside 'Dining Primary_material dropdown' are enabled");
