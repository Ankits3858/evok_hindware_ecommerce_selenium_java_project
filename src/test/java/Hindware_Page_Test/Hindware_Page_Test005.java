...
public class Hindware_Page_Test005 extends Baseclass {
    public WebDriver driver;

    @Test(retryAnalyzer = My_Retry.class)
    public void Hindware_Page_Tank_Type_Dropdown() {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(hw.get_Hindware_Tank_Type_dropdown());
        cu.scroll_down();
        logger.info("successfully clicked on the Hindware Tank_Type dropdown");
        logger.info("VERIFYING the checkboxes inside 'Hindware Tank_Type dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='tank_type']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Tank_Type type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Hindware Tank_Type type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='tank_type']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Tank_Type dropdown' are enabled");
    }
