...
public class Hindware_Page_Test017 extends Baseclass {
    public WebDriver driver;

    @Test(retryAnalyzer = My_Retry.class)
    public void Hindware_Page_Sort_By_Dropdown() {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(hw.get_Hindware_Sort_By_dropdown());
        cu.scroll_down();
        logger.info("successfully clicked on the Hindware Sort_By dropdown");
        logger.info("VERIFYING the checkboxes inside 'Hindware Sort_By dropdown' are Sort_By or not");
        cu.are_checkboxes_dispalyed("(//select[@id='sorter'])[1]//child::option");
        logger.info("ALL checkboxes inside 'Hindware Sort_By type dropdown' are Sort_By");
        logger.info("VERIFYING the checkboxes inside 'Hindware Sort_By type dropdown' enabled or not");
        cu.are_checkboxes_enabled("(//select[@id='sorter'])[1]//child::option");
        logger.info("ALL checkboxes inside 'Hindware Sort_By dropdown' are enabled");
    }
