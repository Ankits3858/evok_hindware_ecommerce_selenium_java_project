...
public class Hindware_Page_Test009 extends Baseclass {
    public WebDriver driver;

    @Test(retryAnalyzer = My_Retry.class)
    public void Hindware_Page_Finish_Type_Dropdown() {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(hw.get_Hindware_Finish_type_dropdown());
        cu.scroll_down();
        logger.info("successfully clicked on the Hindware Finish_Type dropdown");
        logger.info("VERIFYING the checkboxes inside 'Hindware Finish_Type dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='finish_type']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Finish_Type type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Hindware Finish_Type type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='finish_type']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Finish_Type dropdown' are enabled");
    }
    @BeforeMethod
