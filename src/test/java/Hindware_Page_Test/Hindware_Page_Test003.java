...
public class Hindware_Page_Test003 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Hindware_Page_Category_Dropdown()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(hw.get_Hindware_category_dropdown());
        logger.info("successfully clicked on the Hindware category dropdown");
        logger.info("VERIFYING the checkboxes inside 'Hindware category dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='cat']//child::input");
        logger.info("ALL checkboxes inside 'Hindware category type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Hindware category type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='cat']//child::input");
        logger.info("ALL checkboxes inside 'Hindware category dropdown' are enabled");
