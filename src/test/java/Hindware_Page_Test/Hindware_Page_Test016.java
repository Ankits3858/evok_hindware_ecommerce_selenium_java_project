...
public class Hindware_Page_Test016 extends Baseclass {
    public WebDriver driver;

    @Test(retryAnalyzer = My_Retry.class)
    public void Hindware_Page_Display_Dropdown() {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(hw.get_Hindware_Display_dropdown());
        cu.scroll_down();
        logger.info("successfully clicked on the Hindware Display dropdown");
        logger.info("VERIFYING the checkboxes inside 'Hindware Display dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='display']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Display type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Hindware Display type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='display']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Display dropdown' are enabled");
    }
