...
public class Hindware_Page_Test008 extends Baseclass {
    public WebDriver driver;

    @Test(retryAnalyzer = My_Retry.class)
    public void Hindware_Page_Color_Dropdown() {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(hw.get_Hindware_color_dropdown());
        cu.scroll_down();
        logger.info("successfully clicked on the Hindware Color dropdown");
        logger.info("VERIFYING the checkboxes inside 'Hindware Color dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Color type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Hindware Color type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("ALL checkboxes inside 'Hindware Color dropdown' are enabled");
    }
