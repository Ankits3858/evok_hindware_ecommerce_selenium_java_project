...
public class Chairs_page_Test003 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Chairs_page_color_filter()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
      ...
      //  cu.scroll_down();
        ...
        cu.scroll_down();
        cu.click_on_element(cp.get_Chairs_color_dropdown());
        logger.info("successfully clicked on the Chairs color dropdown");
        logger.info("VERIFYING the checkboxes inside 'Chairs color dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("all checkboxes inside 'Chairs color type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Chairs color type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("Successfully close the Chairs color type dropdown");
