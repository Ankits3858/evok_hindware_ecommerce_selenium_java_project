...
public class Chairs_page_Test004 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Chairs_page_FinishType_checkboxes()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       // cu.scroll_down();
        ...
        cu.scroll_down();
        cu.click_on_element(cp.get_Chairs_Finish_type_dropdown());
        logger.info("successfully clicked on the Chairs finish type dropdown");
        logger.info("VERIFYING the checkboxes inside 'Chairs finish type dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='finish_type']//child::input");
        logger.info("all checkboxes inside 'Chairs finish type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Chairs finish type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='finish_type']//child::input");
        logger.info("all checkboxes inside 'Chairs finish type dropdown' are enabled");
        cu.click_on_element(cp.get_Chairs_Finish_type_dropdown());
        logger.info("Successfully close the Chairs finish type dropdown");
    }
    @BeforeMethod
