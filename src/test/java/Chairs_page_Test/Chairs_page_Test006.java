...
public class Chairs_page_Test006 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Chairs_page_sort_by_filter_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        //cu.scroll_down();
        cu.click_on_element(cp.get_chairs_sort_by_dropdown());
        logger.info("successfully clicked on the sort by dropdown");
        logger.info("VERIFYING the options inside 'sort by dropdown' are enabled or not");
        cu.are_checkboxes_enabled("(//select[@id='sorter'])[1]//child::option");
        logger.info("all options inside 'sort by dropdown' are enabled");
        logger.info("VERIFYING the options inside 'sort by dropdown' displayed or not");
        cu.are_checkboxes_dispalyed("(//select[@id='sorter'])[1]//child::option");
        logger.info("all options inside 'sort by dropdown' are displayed");
        cu.click_on_element(cp.get_chairs_sort_by_dropdown());
        logger.info("Successfully close the sort by dropdown");
