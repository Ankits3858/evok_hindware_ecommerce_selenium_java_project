...
public class Sofa_Page_Test007 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Sofa_page_sort_by()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
      ...
      //  cu.scroll_down();
        ...
        cu.click_on_element(sp.get_Sofas_sort_by_dropdown());
        logger.info("successfully clicked on the Sofas Sort By dropdown");
        logger.info("VERIFYING the Options inside 'Sofas Sort By dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("(//select[@id='sorter'])[1]//child::option");
        logger.info("all Options inside 'Sofas Sort By type dropdown' are displayed");
        logger.info("VERIFYING the Options inside 'Sofas Sort By type dropdown' enabled or not");
        cu.are_checkboxes_enabled("(//select[@id='sorter'])[1]//child::option");
        logger.info("ALL Options inside 'sofa Sort By dropdown' are enabled");
