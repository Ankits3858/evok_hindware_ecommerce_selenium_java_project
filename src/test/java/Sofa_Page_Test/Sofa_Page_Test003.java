...
public class Sofa_Page_Test003 extends Baseclass {
    public WebDriver driver;

    @Test(retryAnalyzer = My_Retry.class)
    public void Sofa_Page_category_Dropdown_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(sp.get_sofas_category_dropdown());
        logger.info("successfully clicked on the Sofas category dropdown");
        logger.info("VERIFYING the checkboxes inside 'Sofas category dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='cat']//child::input");
        logger.info("ALL checkboxes inside 'Sofas category type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Sofas category type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='cat']//child::input");
        logger.info("ALL checkboxes inside 'sofa category dropdown' are enabled");
