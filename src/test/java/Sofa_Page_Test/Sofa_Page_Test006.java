...
public class Sofa_Page_Test006 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Sofa_page_Primary_material_filter_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
      ...
      //  cu.scroll_down();
        ...
        cu.click_on_element(sp.get_Sofas_primary_material_dropdown());
        logger.info("successfully clicked on the Sofas Primary material dropdown");
        logger.info("VERIFYING the checkboxes inside 'Sofas Primary material dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("all checkboxes inside 'Sofas Primary material type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Sofas Primary material type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("ALL checkboxes inside 'sofa Primary material dropdown' are enabled");
