...
public class Living_Room_Page_Test006 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void primary_material_filter_checkboxes_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.scroll_down();
        cu.click_on_element(lr.get_primary_material());
        logger.info("successfully clicked on primary material filter");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("all the checkboxes of primary material filter are displayed");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("all the checkboxes of finish type filter are enabled");
        cu.click_on_element(lr.get_primary_material());
        logger.info("successfully closed the primary material filter");
