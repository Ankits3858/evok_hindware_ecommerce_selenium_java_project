...
public class Living_Room_Page_Test004 extends Baseclass {
public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void color_Filter_checkboxes_Test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.scroll_down();
        cu.click_on_element(lr.get_color_filter());
        logger.info("successfully clicked on color filter");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("all the checkboxes inside color filter are enabled");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("all the checkboxes inside the color filter are displayed");
        cu.click_on_element(lr.get_color_filter());
        logger.info("checkboxes closed successfully");
