...
public class Living_Room_Page_Test008 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Living_Room_Page_sort_bY_filter_test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.scroll_down();
        cu.click_on_element(lr.get_LivingRoom_page_sort_by_filter());
        logger.info("Successfully clicked to the sort by filter");
        cu.are_checkboxes_enabled("(//select[@id='sorter'])[1]//child::option");
        logger.info("All the options inside sort by dropdown are enabled");
        cu.are_checkboxes_dispalyed("(//select[@id='sorter'])[1]//child::option");
        cu.click_on_element(lr.get_LivingRoom_page_sort_by_filter());
        logger.info("all the options inside the sort by dropdown are displayed");
