...
public class Storage_Page_Test003 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Storage_Category_dropdown_Test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       // cu.scroll_down();
        ...
        cu.click_on_element(s.get_Storage_category_dropdown());
        logger.info("successfully clicked on the Storage category dropdown");
        logger.info("VERIFYING the checkboxes inside 'Storage category dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='cat']//child::input");
        logger.info("ALL checkboxes inside 'Storage category type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Storage category type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='cat']//child::input");
        logger.info("ALL checkboxes inside 'Storage category dropdown' are enabled");
