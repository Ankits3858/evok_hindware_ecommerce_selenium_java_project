...
public class Storage_Page_Test006 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Storage_Page_primary_material_filter()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       logger.info("Successfully entered inside password field");
        ...
        cu.click_on_element(s.get_Storage_primary_material_dropdown());
        logger.info("successfully clicked on the Storage Primary_Material dropdown");
        logger.info("VERIFYING the checkboxes inside 'Storage Primary_Material dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("ALL checkboxes inside 'Storage Primary_Material type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Storage Primary_Material type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='primary_material']//child::input");
        logger.info("ALL checkboxes inside 'Storage Primary_Material dropdown' are enabled");
