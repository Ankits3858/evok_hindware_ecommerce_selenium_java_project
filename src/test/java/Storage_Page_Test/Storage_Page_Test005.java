...
public class Storage_Page_Test005 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Storage_Finish_type_dropdown_Test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       // cu.scroll_down();
        ...
        cu.click_on_element(s.get_Storage_Finish_type_dropdown());
        logger.info("successfully clicked on the Storage Finish_Type dropdown");
        logger.info("VERIFYING the checkboxes inside 'Storage Finish_Type dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='finish_type']//child::input");
        logger.info("ALL checkboxes inside 'Storage Finish_Type type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Storage Finish_Type type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='finish_type']//child::input");
        logger.info("ALL checkboxes inside 'Storage Finish_Type dropdown' are enabled");
