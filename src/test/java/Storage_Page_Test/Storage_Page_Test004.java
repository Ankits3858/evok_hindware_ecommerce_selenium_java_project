...
public class Storage_Page_Test004 extends Baseclass {
    public WebDriver driver;
    @Test(retryAnalyzer = My_Retry.class)
    public void Storage_Color_dropdown_Test()
    {
...
extent.getTest().log(Status.INFO,"successfully landed to the homepage");
       ...
       // cu.scroll_down();
        ...
        cu.click_on_element(s.get_Storage_color_dropdown());
        logger.info("successfully clicked on the Storage Color dropdown");
        logger.info("VERIFYING the checkboxes inside 'Storage Color dropdown' are displayed or not");
        cu.are_checkboxes_dispalyed("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("ALL checkboxes inside 'Storage Color type dropdown' are displayed");
        logger.info("VERIFYING the checkboxes inside 'Storage Color type dropdown' enabled or not");
        cu.are_checkboxes_enabled("//div[@data-role='ln_collapsible' and @attribute='color']//child::input");
        logger.info("ALL checkboxes inside 'Storage Color dropdown' are enabled");
